...
static NFA_t* NFA_concat(NFA_t* restrict, NFA_t* restrict);
static NFA_t* NFA_union(NFA_t* restrict, NFA_t* restrict);
static int NFA_star(NFA_t*);
static NFA_t* NFA_init(ssize_t);
...
NFA_t* NFA_build(const node_t* parse_tree){
    ...
    switch(parse_tree->op){
        ...
        case NONE:
            if ((nfa = NFA_simple(parse_tree->c)) == NULL)
                return NULL;
            break;
...
static NFA_t* NFA_init(ssize_t n_states){
    NFA_t* nfa;
    if ((nfa = calloc(1, sizeof(NFA_t))) == NULL ||
        (nfa->states = calloc(n_states, sizeof(state_t))) == NULL)
        return NULL;
...
static NFA_t* NFA_simple(char c){
    NFA_t* nfa;
    if ((nfa = NFA_init(2)) == NULL)
        return NULL;
    
    if (NFA_state_init(&nfa->states[0], false) != 0 ||
        NFA_state_init(&nfa->states[1], true) != 0 ||
        NFA_state_addsymbol(&nfa->states[0], c, 1) != 0){
            NFA_destroy(nfa);
            return NULL;
    }
...
static NFA_t* NFA_concat(NFA_t* restrict nfa1, NFA_t* restrict nfa2){
    NFA_t* nfa;
    if ((nfa = NFA_init(nfa1->states_len + nfa2->states_len - 1)) == NULL)
        return NULL;
    ...
    for (i=0; i<nfa1->states_len; ++i){
        if ( nfa->states[i].final ){
            for (j=0; j<nfa2->states[0].len; ++j)
                if (NFA_state_addsymbol(&nfa->states[i], nfa2->states[0].charset[j], nfa2->states[0].mapped_state[j] + nfa1->states_len - 1) != 0){
                    NFA_destroy(nfa);
                    return NULL;
                }
...
static NFA_t* NFA_union(NFA_t* restrict nfa1, NFA_t* restrict nfa2){
    NFA_t* nfa;
    if ((nfa = NFA_init(nfa1->states_len + nfa2->states_len - 1)) == NULL)
        return NULL;
    ...
    for (i=0; i<nfa2->states[0].len; ++i)
        if (NFA_state_addsymbol(&nfa->states[0], nfa2->states[0].charset[i], nfa2->states[0].mapped_state[i] + nfa1->states_len - 1) != 0){
            NFA_destroy(nfa);
            return NULL;
        }
    ...
    for (i=1; i<nfa->states_len; ++i){
        if (nfa->states[i].final){
            ...
            for (j=0; j<nfa->states[0].len; ++j)
                if (NFA_state_addsymbol(&nfa->states[i], nfa->states[0].charset[j], nfa->states[0].mapped_state[j])){
                    NFA_destroy(nfa);
                    return -1;
                }
        }
